...
void inicializa(matriz *a, int m, int n)
  {
    ...
    for (i = 0; i < n; i++)
      { t = (ap_elemento)MALLOC(sizeof(elemento));
        ...
        t->val = 0;
        t->ac = t; t->ab = t;
        /* Insere logo à esquerda da super-cabeça: */
        t->esq = r->esq; t->dir = r;
        t->dir->esq = t;
        t->esq->dir = t;
        a->ccol[i] = t;
...
void libera(matriz *a) {
  /* Libera toda a memória dinâmica ocupada por uma matriz */
    ...
    for (i = 0; i < lmax; i++)
      FREE(a->clin[i]);
    /* |i <= cmax| para liberar a super-cabeça também */
    for (i = 0; i <= cmax; i++)
      FREE(a->ccol[i]);
    FREE(a->clin);
    FREE(a->ccol);
...
void soma(matriz *a, matriz *b, matriz *s)
  {
    ...
    for (i = 0; i < lmax; i++) {
      coluna = s->ccol[i];
      /* Caminha a lista da i-ésima linha */
      ...
      for (j = 0; j < max; j++) {
        ...
        for (a_atual = a->clin[i]->dir, b_atual = b->ccol[j]->ab;
          ...
          if (a_atual->col < b_atual->lin) {
            /* a[i,j] * 0 = 0, não somar esse termo */
            a_atual = a_atual->dir;
          } else if (b_atual->lin < a_atual->col) {
            /* 0 * b[i,j] = 0, não somar esse termo */
            b_atual = b_atual->ab;
          } else {
            v += a_atual->val * b_atual->val;
            a_atual = a_atual->dir;
